            ...
            <form action="{{ route('users.update', $user) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    @error('password')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-gray-500">Mínimo 8 caracteres. Solo completa si deseas cambiar la contraseña.</p>
                </div>
                    ...
                    <select name="sucursal_id" 
                        ...
                        @foreach($branches as $branch)
                            <option value="{{ $branch->id }}" 
                                    {{ old('sucursal_id', $user->sucursal_id) == $branch->id ? 'selected' : '' }}>
